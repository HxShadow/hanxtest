#ssh no-password
ssh-keygen -t rsa
ssh-copy-id -i id_rsa.pub hanx.tpddns.cn -p 2008
...
# vim setup
git clone https://github.com/hanx-phy/.vim.git
sudo yum install vim-X11.x86_64 

#Change repo & upgrade
sudo yum clean all && sudo yum makecache && sudo yum -y update &&  sudo yum -y upgrade
...
# ntfs-3g & support Windows
# For now, 2019/10/13, it does not support for CentOS 8. We need to compile it.
